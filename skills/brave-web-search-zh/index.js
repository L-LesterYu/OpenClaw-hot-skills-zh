...
function getSearchApiKey() {
  ...
  if (!key) {
    throw new Error(
      'BRAVE_SEARCH_API_KEY 环境变量未设置。' +
      '请使用以下命令导出: export BRAVE_SEARCH_API_KEY=your_key_here'
    );
  }
  return key;
}
...
function getAnswersApiKey() {
  ...
  if (!key) {
    throw new Error(
      'BRAVE_ANSWERS_API_KEY 环境变量未设置。' +
      '请使用以下命令导出: export BRAVE_ANSWERS_API_KEY=your_key_here'
    );
  }
  return key;
}
...
async function braveSearch({ query, count = 10, freshness }) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Brave Search API 错误 ${res.status}: ${err}`);
  }
  ...
  if (results.length === 0) {
    return { summary: '未找到该查询的网页结果。', results: [] };
  }
...
async function braveAnswer({ query }) {
  ...
  if (!searchRes.ok) {
    ...
    throw new Error(`Brave Search API 错误 ${searchRes.status}: ${err}`);
  }
  ...
  if (!summarizerKey) {
    ...
    return {
      query,
      answer: null,
      note: '此查询没有可用的 AI 摘要。返回前几个网页结果作为替代。',
  ...
  if (!summaryRes.ok) {
    ...
    throw new Error(`Brave Summarizer API 错误 ${summaryRes.status}: ${err}`);
  }
...
if (!handler) {
  console.error(JSON.stringify({
    error: `未知命令: ${command}。可用命令: ${Object.keys(handlers).join(', ')}`
  }));
  process.exit(1);
}
